...
function toDoubleByteChar(text: string): string {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    } else if (charCode === 32) { // Space converted to full-width space
...
function toSingleByteChar(text: string): string {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (charCode >= 65281 && charCode <= 65374) {
      result += String.fromCharCode(charCode - 65248)
    } else if (charCode === 12288) { // Full-width space converted to space
      result += ' '
    } else {
      result += text.charAt(i)
    }
...
export default function ByteConverter() {
  ...
  useEffect(() => {
    if (debouncedText) {
      const result = conversionType === 'TO_FULL_WIDTH' 
        ? toDoubleByteChar(debouncedText) 
        : toSingleByteChar(debouncedText)
    ...
    <div className="w-full flex flex-col gap-2">
      ...
      <div className="flex flex-col md:flex-row gap-1 w-full md:min-h-[500px] md:h-[64vh]">
        <div className="w-full md:w-1/2 min-h-[250px] h-full">
          <ReactEditor 
            title="Source Text" 
            className="min-h-[64vh] md:min-h-[100%]" 
            value={sourceText} 
            onChange={setSourceText} 
            storageKey="byte-converter-source" 
          />
        </div>
